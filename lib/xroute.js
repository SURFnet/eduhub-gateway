...
const endpointRe = /[a-zA-Z0-9.-]+/
const endpointHeaderRe = new RegExp(`^\\s*endpoint\\s*=(\\s*${endpointRe.source}(?:\\s*,\\s*${endpointRe.source}\\s*)*)$`)

const decode = (xroute, throwOnMalformed = false) => {
  if (xroute) {
    const [, endpoints] = endpointHeaderRe.exec(xroute) || []

    if (endpoints) {
      return lodash.uniq(endpoints.trim().split(/\s*,\s*/))
    } else if (throwOnMalformed) {
      throw new MalformedHeader(`Malformed X-Route header '${xroute}'`)
    }
  }
  return null
}
