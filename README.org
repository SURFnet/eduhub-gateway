...
Requests will be validated against the [[./ooapiv5.json][OOAPI specification V5]] or [[./ooapiv6.json][OOAPI
specification V6]] using the ~ooapi-request-validator~ policy.

#+begin_src yaml
    - ooapi-request-validator:
        - action:
            apiSpecs:
                '5': 'ooapiv5.json'
                '6': 'ooapiv6.json'
...
When the requested version can not be derived from the "Accept"
header, it will respond with a ~406 Not Acceptable~ response.

When the request does not match one of the specification, it will
respond with a ~400 Bad Request~ response.

The spec files can be generated using:

#+begin_src sh
  make ooapiv5.json ooapiv6.json
#+end_src

Which will generate a versions of OOAPI specification that exclude the
response schemas since the full specifications are not compatible with
the validation library used.

You will need to have [[https://stedolan.github.io/jq/][jq]] installed.
